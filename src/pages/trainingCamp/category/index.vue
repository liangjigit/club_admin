<template>
  <div>
    ...
    >
      ...
      <!-- 排序 -->
      <span slot="categorySort" slot-scope="text, record">
        <editable-cell
          :text="text"
          @change="onCellChange(record.id, 'categorySort', $event)"
        />
      </span>
      <!-- 操作 -->
      <template slot="action" slot-scope="text, record">
        ...
        <span v-show="!Boolean(record.status)">
          ...
          <a
            @click="
              showEditModal(record.id, 'categoryName', record.categoryName)
            "
          >
            编辑
          </a>
    ...
    >
      ...
      >
        <a-form-model-item ref="name" label="分类名称" prop="name">
          <a-input v-model="form.name" placeholder="请输入分类名称，10字以内" />
        </a-form-model-item>
...
const columns = [
  ...
  },
  {
    ...
    title: "排序",
    scopedSlots: { customRender: "categorySort" }
  },
  {
    dataIndex: "status",
    title: "状态",
...
export default {
  ...
  computed: {
    ...mapState("trainingCampCategory", {
      list: state => state.categories,
      currentPage: state => state.pageNum,
      pageSize: state => state.pageSize,
      totalPages: state => state.totalSize
    })
  },
  created() {
    this.getData();
  ...
  methods: {
    ...mapActions("trainingCampCategory", [
      "getCategories",
      "changeCategorySort",
    ...
    async changeStatus(id, dataIndex, value) {
      ...
      const response = await this.changeCategoryStatus({
        status,
        id,
        dataIndex
      });
    ...
    async onCellChange(id, dataIndex, value) {
      ...
      const response = await this.changeCategorySort({
        categorySort: value,
        id,
        dataIndex
      });
      ...
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          let response = this.isEdit
            ? this.editForCategory()
            : this.createForCategory();
    ...
    // 编辑分类
    editForCategory() {
      const { name: categoryName, id, dataIndex } = this.form;
      return this.editCategory({ categoryName, id, dataIndex });
    },
    // 新增分类
    createForCategory() {
      return this.createCategory({ categoryName: this.form.name });
    },
    // 取消、隐藏新增/编辑Modal
    cancelCreate() {
      this.visible = false;
