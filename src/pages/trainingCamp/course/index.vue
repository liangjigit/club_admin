	...
	<div>
		<div class="functional-area" slot="actions">
			<a-form layout="inline" :form="search" @submit="searchSubmit">
				<a-form-item>
					<a-button type="primary" @click="$router.push({ path: '/trainingCamp/course/create' })">新增课程
					</a-button>
				</a-form-item>
				...
				<a-form-item label="分类">
					<a-select show-search :allowClear="true" :show-arrow="true" :not-found-content="'未找到'"
						:placeholder="'请输入分类'" :default-value="search.classify" v-model="search.classify"
						style="width: 200px">
						<a-select-option v-for="item in allCategories" :key="item.categoryName"
				...
				<a-form-item label="状态">
					<a-select :allowClear="true" placeholder="请选择状态" :default-value="search.status"
						...
						<a-select-option :value="item.key" v-for="item in courseStatus" :key="item.value">
							{{ item.value }}
						</a-select-option>
					</a-select>
				</a-form-item>
		...
		<a-table :scroll="{ x: 1300 }" :columns="columns" :data-source="courses" bordered :pagination="false"
			...
			<template slot="status" slot-scope="text, record">
				<span v-for="item in courseStatus" :key="item.value" v-show="item.key === record.status">
					{{ item.value }}
				</span>
			</template>
			<!-- 排序 -->
			<span slot="courseSort" slot-scope="text, record">
				<editable-cell :text="record.courseSort" @change="onCellChange(record.id, 'courseSort', $event)" />
			</span>
			<!-- 操作 -->
			<template slot="action" slot-scope="text, record">
				...
				<br />
				<router-link :to="{ name: 'trainingCampCourseDetail', params: { id: record.id } }">
					查看
				</router-link>
				<br />
				<router-link :to="{ name: 'trainingCampCoursePoster', params: { id: record.id } }">
					分享
				</router-link>
				<br />
				<!-- 下架状态下显示编辑和删除 -->
				<span v-if="record.status == 0">
					<router-link :to="{ name: 'trainingCampCourseEdit', params: { id: record.id } }">
						编辑
					</router-link>
					<br />
					<a-popconfirm v-if="courses.length" title="确定删除?" ok-text="删除" cancel-text="取消"
						@confirm="() => onDelete(record.id, 'deleteCourse')">
						<a href="javascript:;">删除</a>
					</a-popconfirm>
					<br />
				</span>
				<a :href="record.qrcodeUrl" :download="record.courseCode + '.png'">下载二维码</a>
				<br />
				<router-link :to="{
		...
		<div style="margin-top:20px;textAlign:right">
			<a-pagination :default-current="1" v-model="pageNum" :total="totalSize" :pageSize="pageSize"
	...
	export default {
		...
		methods: {
			...
			async onDelete(id, dataIndex) {
				...
				const response = await this.deleteCourse({
					id
				});
