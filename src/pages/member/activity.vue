<template>
    ...
    <a-row :gutter="16">
        <a-col
                v-for="(item, index) in userActivitys"
                :key="index"
                class="gutter-row"
                :xl="12"
                :lg="12"
            ...
            >
                    ...
                    <ul class="field_list">
                        <li v-for="(item, index) in regForm.fieldList" :key="index">
                            ...
                            <a-popconfirm
                                    ...
                                    ok-text="删除"
                                    cancel-text="取消"
                                    @confirm="() => onDeleteFields(item, 'reg', index)"
                            >
                ...
                <div class="form_item">
                    ...
                    <p v-if="regForm.isCoupon" class="form_gift">
                        每个用户可获得
                        <a-input placeholder="优惠券ID" v-model="templateCode"
                                 style="width:200px; margin-left:6px; margin-right: 6px;"/>
                        ...
                        <ul class="coupon_list">
                            <li v-for="(item,index) in regForm.templateList" :key="index">
                                ...
                                <a-popconfirm
                                        ...
                                        ok-text="删除"
                                        cancel-text="取消"
                                        @confirm="() => onDeleteCoupon('reg', index)"
                                >
                                    <a href="javascript:;">删除</a>
                                </a-popconfirm>
                            </li>
            ...
            >
                ...
                <div v-if="userInfoForm.userInfoConfig.status">
                    <div class="form_item" style="margin-left:160px;">
                        <span class="form_label">设置信息项：</span>
                        <a-select :allowClear="true" v-model="userInfoField"
                                  placeholder="请选择" style="width:200px">
                            <a-select-option :value="item.id" :key="item.id"
                                             v-for="item in userInfoFieldList">
                                {{item.fieldName}}
                            </a-select-option>
                        </a-select>
                        <a-button type="primary"
                                  @click="addUserInfoFields()"
                        ...
                        <ul class="field_list">
                            <li v-for="(item,index) in userInfoForm.userInfoConfig.fieldList" :key="index">
                                ...
                                <a-popconfirm
                                        ...
                                        ok-text="删除"
                                        cancel-text="取消"
                                        @confirm="() => onDeleteFields(item, 'userInfo', index)"
                                >
                                    <a href="javascript:;">删除</a>
                                </a-popconfirm>
                            </li>
                    ...
                    <div class="form_item">
                        ...
                        <p v-if="userInfoForm.userInfoConfig.isCoupon">
                            <span class="form_span" style="margin-left: 180px">每个用户可获得</span>
                            <a-input placeholder="优惠券ID" v-model="userInfoTemplateCode"
                                     style="width:200px; margin-left:6px; margin-right: 6px;"/>
                            <a-button
                                    type="primary"
                                    @click="searchCouponTemplate('userInfo')"
                                    style="margin-left:10px"
                                    v-if="userInfoForm.userInfoConfig.templateList.length < 1"
                            >添加
                            </a-button>
                            <ul class="coupon_list" style="margin-left: 180px">
                                <li v-for="(item,index) in userInfoForm.userInfoConfig.templateList" :key="index">
                                    <span class="form_span">{{ item.couponName }}</span>
                                    <a-form-model-item :prop="'userInfoConfig.templateList.' + index +'.couponNum'" :rules="userInfoRules.couponNum" class="inline">
                                        <a-input v-model="item.couponNum" style="margin:0 6px; width: 100px;" />
                                    </a-form-model-item>
                                    <span class="form_span">张</span>
                                    <a-popconfirm
                                            ...
                                            ok-text="删除"
                                            cancel-text="取消"
                                            @confirm="() => onDeleteCoupon('userInfo', index)"
                                    >
                                        <a href="javascript:;">删除</a>
                                    </a-popconfirm>
                                </li>
                ...
                <div v-if="userInfoForm.userSpouseConfig.status">
                    <div class="form_item" style="margin-left:160px;">
                        <span class="form_label">设置信息项：</span>
                        <a-select :allowClear="true" v-model="userSpouseField"
                                  placeholder="请选择" style="width:200px">
                            <a-select-option :value="item.id" :key="item.id"
                                             v-for="item in userSpouseFieldList">
                                {{item.fieldName}}
                            </a-select-option>
                        </a-select>
                        ...
                        <ul class="field_list">
                            <li v-for="(item,index) in userInfoForm.userSpouseConfig.fieldList" :key="index">
                                ...
                                <a-popconfirm
                                        ...
                                        ok-text="删除"
                                        cancel-text="取消"
                                        @confirm="() => onDeleteFields(item, 'userSpouse', index)"
                                >
                                    <a href="javascript:;">删除</a>
                                </a-popconfirm>
                            </li>
                    ...
                    <div class="form_item">
                        ...
                        <p v-if="userInfoForm.userSpouseConfig.isCoupon">
                            <span class="form_span" style="margin-left: 180px">每个用户可获得</span>
                            <a-input placeholder="优惠券ID" v-model="userSpouseTemplateCode"
                                     style="width:200px; margin-left:6px; margin-right: 6px;"/>
                            <a-button
                                    type="primary"
                                    @click="searchCouponTemplate('userSpouse')"
                                    style="margin-left:10px"
                                    v-if="userInfoForm.userSpouseConfig.templateList.length < 1"
                            >添加
                            </a-button>
                            <ul class="coupon_list" style="margin-left: 180px">
                                <li v-for="(item,index) in userInfoForm.userSpouseConfig.templateList" :key="index">
                                    <span class="form_span">{{ item.couponName }}</span>
                                    <a-form-model-item :prop="'userSpouseConfig.templateList.' + index +'.couponNum'" :rules="userInfoRules.couponNum" class="inline">
                                        <a-input v-model="item.couponNum" style="margin:0 6px; width: 100px;" />
                                    </a-form-model-item>
                                    <span class="form_span">张</span>
                                    <a-popconfirm
                                            ...
                                            ok-text="删除"
                                            cancel-text="取消"
                                            @confirm="() => onDeleteCoupon('userSpouse', index)"
                                    >
                                        <a href="javascript:;">删除</a>
                                    </a-popconfirm>
                                </li>
                ...
                <div v-if="userInfoForm.userBabyConfig.status">
                    <div class="form_item" style="margin-left:160px;">
                        <span class="form_label">设置信息项：</span>
                        <a-select :allowClear="true" v-model="userBabyField"
                                  placeholder="请选择" style="width:200px">
                            <a-select-option :value="item.id" :key="item.id"
                                             v-for="item in userBabyFieldList">
                                {{item.fieldName}}
                            </a-select-option>
                        </a-select>
                        ...
                        <ul class="field_list">
                            <li v-for="(item,index) in userInfoForm.userBabyConfig.fieldList" :key="index">
                                ...
                                <a-popconfirm
                                        ...
                                        ok-text="删除"
                                        cancel-text="取消"
                                        @confirm="() => onDeleteFields(item, 'userBaby', index)"
                                >
                                    <a href="javascript:;">删除</a>
                                </a-popconfirm>
                            </li>
                    ...
                    <div class="form_item">
                        ...
                        <p v-if="userInfoForm.userBabyConfig.isCoupon">
                            <span class="form_span" style="margin-left: 180px">每个用户可获得</span>
                            <a-input placeholder="优惠券ID" v-model="userBabyTemplateCode"
                                     style="width:200px; margin-left:6px; margin-right: 6px;"/>
                            <a-button
                                    type="primary"
                                    @click="searchCouponTemplate('userBaby')"
                                    style="margin-left:10px"
                                    v-if="userInfoForm.userBabyConfig.templateList.length < 1"
                            >添加
                            </a-button>
                            <ul class="coupon_list" style="margin-left: 180px">
                                <li v-for="(item,index) in userInfoForm.userBabyConfig.templateList" :key="index">
                                    <span class="form_span">{{ item.couponName }}</span>
                                    <a-form-model-item :prop="'userBabyConfig.templateList.' + index +'.couponNum'" :rules="userInfoRules.couponNum" class="inline">
                                        <a-input v-model="item.couponNum" style="margin:0 6px; width: 100px;" />
                                    </a-form-model-item>
                                    <span class="form_span">张</span>
                                    <a-popconfirm
                                            ...
                                            ok-text="删除"
                                            cancel-text="取消"
                                            @confirm="() => onDeleteCoupon('userBaby', index)"
                                    >
                                        <a href="javascript:;">删除</a>
                                    </a-popconfirm>
                                </li>
                    ...
                    <a-form-model-item v-if="userInfoForm.userBabyConfig.isAddBaby" style="margin-left: 270px;margin-top:20px">
                        <a-textarea v-model="userInfoForm.userBabyConfig.addBabyDesc" rows="4" cols="4"
                                    placeholder="新增宝贝说明"></a-textarea>
                    </a-form-model-item>
                </div>
            </a-form-model>
        </a-modal>
        ...
        >
            ...
            >
                ...
                <a-form-model-item style="margin-left: 100px;">
                    ...
                    <div>
                        ...
                        <p v-if="inviteForm.isCoupon">
                            每个用户可获得
                            <a-input placeholder="优惠券ID" v-model="inviteTemplateCode"
                                     style="width:200px; margin-left:6px; margin-right: 6px;"/>
                            ...
                            <ul class="coupon_list">
                                <li v-for="(item,index) in inviteForm.templateList" :key="index">
                                    ...
                                    <a-popconfirm
                                            ...
                                            ok-text="删除"
                                            cancel-text="取消"
                                            @confirm="() => onDeleteCoupon('invite', index)"
                                    >
                                        <a href="javascript:;">删除</a>
                                    </a-popconfirm>
                                </li>
                ...
                <a-form-model-item label="海报图片" extra="png/jpg格式，2M以内">
                    <upload-file @uploadPic="uploadPic" :img="inviteForm.image"
                                 v-model="inviteForm.image"></upload-file>
                </a-form-model-item>
            </a-form-model>
        </a-modal>
        ...
        >
            ...
            <a-form-model
                    ref="userActivityForm"
                    :label-col="labelCol"
                    :wrapper-col="wrapperColActivity"
                    :model="userActivityForm"
            ...
            >
                ...
                <a-form-model-item style="margin-left: 100px; border-bottom: 1px dotted #cccccc" v-for="(item2, index2) in userActivityConfigs"
                    ...
                    <div>
                        参加
                        <a-select :allowClear="true" v-model="item2.activityId"
                                  placeholder="请选择" style="width:200px">
                            <a-select-option :value="item.id" :key="item.id" :class="item.disabled?'hidden':''"
                        ...
                        <a-popconfirm
                                ...
                                ok-text="删除"
                                cancel-text="取消"
                                @confirm="() => onDeleteActivity(index2)"
                        >
                    ...
                    <div>
                        ...
                        <p v-if="item2.isCoupon">
                            每个用户可获得
                            <a-input placeholder="优惠券ID" v-model="activityTemplateCode[index2]"
                                     style="width:200px; margin-left:6px; margin-right: 6px;"/>
                            ...
                            <ul>
                                <li v-for="(item, index) in item2.templateList" :key="index">
                                    {{ item.couponName }}
                                    <a-input-number :min="1" :precision="0" v-model="item.couponNum"
                                             style="width:50px; margin-left:6px; margin-right: 6px;"/>
                                    张
                                    <a-popconfirm
                                            title="确定删除?"
                                            ok-text="删除"
                                            cancel-text="取消"
                                            @confirm="() => onDeleteActivityCoupon(index, index2)"
                                    >
                                        <a href="javascript:;">删除</a>
                                    </a-popconfirm>
                                </li>
  ...
  export default {
    ...
    data() {
      return {
        labelCol: {span: 6},
        wrapperCol: {span: 16},
        regWrapperCol: {span: 20},
        wrapperColActivity: {span: 18},
        regVisible: false,
    ...
    methods: {
      ...
      async searchCouponTemplate(value) {
        if (value == 'reg') {
          if (this.templateCode == null || this.templateCode == '') {
            ...
            const response = await this.getCouponTemplates({"templateCode": this.templateCode});
          ...
          if (this.userInfoTemplateCode == null || this.userInfoTemplateCode == '') {
            ...
            const response = await this.getCouponTemplates({"templateCode": this.userInfoTemplateCode});
            if (response.code != 200) {
              this.$message.error({
                content: response.msg,
              })
          ...
          if (this.userSpouseTemplateCode == null || this.userSpouseTemplateCode == '') {
            ...
            const response = await this.getCouponTemplates({"templateCode": this.userSpouseTemplateCode});
            if (response.code != 200) {
              this.$message.error({
                content: response.msg,
              })
          ...
          if (this.userBabyTemplateCode == null || this.userBabyTemplateCode == '') {
            ...
            const response = await this.getCouponTemplates({"templateCode": this.userBabyTemplateCode});
            if (response.code != 200) {
              this.$message.error({
                content: response.msg,
              })
          ...
          if (this.inviteTemplateCode == null || this.inviteTemplateCode == '') {
            ...
            const response = await this.getCouponTemplates({"templateCode": this.inviteTemplateCode});
            if (response.code != 200) {
              this.$message.error({
                content: response.msg,
              })
      ...
      async addActivityCoupon(activityTemplateCode, index) {
        if (activityTemplateCode[index] == null || activityTemplateCode[index] == '') {
          ...
          const response = await this.getCouponTemplates({"templateCode": activityTemplateCode[index]});
          if (response.code != 200) {
            this.$message.error({
              content: response.msg,
            })
      ...
      userInfoSubmit() {
        this.$refs.userInfoForm.validate(valid => {
          if (valid) {
            ...
            } else if (this.userInfoForm.userInfoConfig.isCoupon && this.userInfoForm.userInfoConfig.templateList.length <= 0) {
              this.$message.error({
                content: "【我的信息】请选择优惠券",
              });
            ...
            } else if (this.userInfoForm.userSpouseConfig.isCoupon && this.userInfoForm.userSpouseConfig.templateList.length <= 0) {
              this.$message.error({
                content: "【我的TA】请选择优惠券",
              });
            ...
            } else if (this.userInfoForm.userBabyConfig.isCoupon && this.userInfoForm.userBabyConfig.templateList.length <= 0) {
              this.$message.error({
                content: "【我的宝贝】请选择优惠券",
              });
            } else {
      ...
      async getUserInfoFieldList() {
        const type = 1;
        const response = await this.getUserFileds({type});
      ...
      async getFieldList() {
        const type = 4;
        let response = await this.getUserFileds({type});
