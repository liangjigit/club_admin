<template>
	<div>
		<div class="functional-area" slot="actions">
			<a-form layout="inline" :form="search" @submit="searchSubmit">
				<a-form-item label="用户手机号">
					<a-input placeholder="用户手机号" v-model="search.phone" :allowClear="true" />
				</a-form-item>
				<a-form-item label="姓名">
					<a-input placeholder="姓名" v-model="search.realname" :allowClear="true" />
				</a-form-item>
				<a-form-item label="会员卡号">
					<a-input placeholder="会员卡号" v-model="search.memberNo" :allowClear="true" />
				</a-form-item>
				<a-form-item label="类型">
					<a-select :allowClear="true" placeholder="请选择" :default-value="search.type" v-model="search.type"
						style="width: 100px">
				...
				<a-form-item label="积分来源">
					<a-select :allowClear="true" placeholder="请选择" :default-value="search.source"
						v-model="search.source" style="width: 150px">
						<a-select-option v-for="item in source" :value="item.key" :key="item.item">{{ item.value }}
						</a-select-option>
					</a-select>
				</a-form-item>
		...
		<a-table :scroll="{ x: 1000 }" :columns="columns" :data-source="userIntegralFlows" bordered :pagination="false"
			...
			<template slot="typeSlot" slot-scope="text, record">
				<span v-for="item in type" :key="item.value" v-show="item.key === record.type">
					{{ item.value }}
				</span>
			</template>

			<template slot="sourceSlot" slot-scope="text, record">
				<span v-for="item in source" :key="item.value" v-show="item.key === record.source">
					{{ item.value }}
				</span>
			</template>
		</a-table>
		...
		<div style="margin-top:20px;textAlign:right">
			<a-pagination :default-current="1" v-model="currentPage" :total="totalSize" :pageSize="pageSize"
