<template>
  <div>
    ...
    >
      ...
      <template slot="collectionCount" slot-scope="text, record">
        <span v-if="record.collectionCount <= 0">
        ...
        <router-link
          ...
          :to="{
            ...
            params: { id: record.articleId, title: record.title }
          }"
        >
          {{ record.collectionCount }}
        </router-link>
...
export default {
  ...
  methods: {
    ...mapActions("cupStatistics", ["getStatistics", "exportStatistics"]),
    init() {
      if (this.$route.name != "cupStatisticsIndex") {
        return;
      }
      this.getData();
    },
    ...
    async exportData() {
      ...
      exportFile(response, "CUP有料数据统计"); // 导出文件
