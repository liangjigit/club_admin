  ...
  <div>
    <div class="functional-area" slot="actions">
      <a-form layout="inline" :form="search" @submit="searchSubmit">
        <a-form-item>
          <a-button
            type="primary"
            @click="$router.push({ path: '/cup/article/create' })"
            >新增文章</a-button
          >
        </a-form-item>
        ...
        <a-form-item label="专家姓名">
          <a-select
            ...
            :placeholder="'请输入专家名称'"
            :default-value="search.expertId"
            v-model="search.expertId"
            style="width: 200px"
          >
            <a-select-option
    ...
    >
      ...
      <span slot="articleSort" slot-scope="text, record">
        <editable-cell
          :text="record.articleSort"
          @change="onCellChange(record.id, 'articleSort', $event)"
        />
      </span>
      <!-- 操作 -->
      <template slot="action" slot-scope="text, record">
        ...
        <br />
        <router-link
          :to="{ name: 'cupArticleDetail', params: { id: record.id } }"
        >
          查看
        </router-link>
        <br />
        <router-link
          :to="{ name: 'cupArticlePoster', params: { id: record.id } }"
        >
          分享
        </router-link>
        <br />
        <a :href="record.qrcodeUrl" :download="record.articleCode + '.png'"
          >下载二维码</a
        >
        <br />
        <!-- 下架状态下显示编辑和删除 -->
        <span v-if="record.status == 0">
          <router-link
            :to="{ name: 'cupArticleEdit', params: { id: record.id } }"
          >
            编辑
          </router-link>
          <br />
...
const columns = [
  ...
  },
  {
    ...
    key: "articleSort",
    title: "排序",
    scopedSlots: { customRender: "articleSort" },
    width: 130
  },
  {
    title: "操作",
...
export default {
  ...
  methods: {
    ...
    async onDelete(id, dataIndex) {
      ...
      const response = await this.deleteArticle({ id });
    ...
    async getData(page, pageSize) {
      let { title, expertId, status, createTime } = this.search;
      // 格式化时间
