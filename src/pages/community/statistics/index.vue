  ...
  <div>
    ...
    >
      ...
      <template slot="applyCount" slot-scope="text, record">
        <span v-if="record.applyCount <= 0">{{ record.applyCount }}</span>
        <router-link
          v-else
          :to="{
            name: 'communitySignUpIndex',
            params: { activityCode: record.activityCode }
          }"
        >
          {{ record.applyCount }}
        </router-link>
        / {{ record.applyCancelCount }}
      </template>
    </a-table>
    <!-- 分页器 -->
...
export default {
  ...
  methods: {
    ...mapActions("communityStatistic", ["exportStatistics", "getStatistics"]),
    init() {
      if (this.$route.name != "communityStatisticsIndex") {
        return;
      }
      this.getData();
    },
    ...
    async exportData() {
      ...
      exportFile(response, "社区活动数据统计"); // 导出文件
