...
export default {
  ...
  methods: {
    ...mapActions("cupStatistics", ["getStatistics", "exportBrartsStatistics"]),
    init() {
      if (this.$route.name != "brartsStatisticsIndex") {
        return;
      }
      this.getData();
    },
    ...
    async exportData() {
      ...
      const response = await this.exportBrartsStatistics({
        pageNum,
        pageSize,
        title
      });
      exportFile(response, "BRARTS数据统计"); // 导出文件
