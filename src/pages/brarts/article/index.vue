  ...
  <div>
    <div class="functional-area" slot="actions">
      <a-form layout="inline" :form="search" @submit="searchSubmit">
        <a-form-item>
          <a-button
            type="primary"
            @click="$router.push({ path: '/brarts/article/create' })"
            >新增文章</a-button
          >
        </a-form-item>
    ...
    >
      ...
      <template slot="action" slot-scope="text, record">
        ...
        <br />
        <router-link
          :to="{ name: 'brartsArticleDetail', params: { id: record.id } }"
        >
          查看
        </router-link>
        <br />
        <router-link
          :to="{ name: 'brartsArticlePoster', params: { id: record.id } }"
        >
          分享
        </router-link>
        <br />
        ...
        <span v-if="record.status == 0">
          <router-link
            :to="{ name: 'brartsArticleEdit', params: { id: record.id } }"
          >
            编辑
          </router-link>
          <br />
...
export default {
  ...
  methods: {
    ...
    // 初始化数据
    init() {
      if (this.$route.name != "brartsArticleIndex") {
        return false;
      }
      this.getData(); // 获取列表数据
    },
    ...
    async getData(page, pageSize) {
      this.loading = true;
      let { title, status, createTime } = this.search;
      // 格式化时间
